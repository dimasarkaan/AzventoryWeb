 ...
 */
class TesServisInventarisTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        $catId = \App\Models\Category::factory()->create(['name' => 'Oli'])->id;
        Sparepart::factory()->create(['category_id' => $catId]);

        $result = $this->service->getFilteredSpareparts(['category' => 'Oli']);
        foreach ($result as $item) {
            $this->assertEquals($catId, $item->category_id);
        }
    }
