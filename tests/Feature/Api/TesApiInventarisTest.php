 ...
 */
class TesApiInventarisTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function semua_endpoint_ditolak_tanpa_token()
    {
        ...
        $this->getJson("/api/v1/inventory/{$sparepart->uuid}")->assertStatus(401);
        $this->putJson("/api/v1/inventory/{$sparepart->uuid}", [])->assertStatus(401);
        $this->deleteJson("/api/v1/inventory/{$sparepart->uuid}")->assertStatus(401);
        $this->putJson("/api/v1/inventory/{$sparepart->uuid}/adjust-stock", [])->assertStatus(401);
    }
    ...
    public function index_dengan_filter_category_mengembalikan_data_sesuai()
    {
        ...
        $response = $this->getJson('/api/v1/inventory?category='.$cat->name);

        $response->assertStatus(200);
        $data = $response->json('data');
        $this->assertCount(1, $data);
        $this->assertEquals($cat->name, $data[0]['category']);
    }
    ...
    public function store_gagal_jika_field_wajib_tidak_diisi()
    {
        ...
        $this->postJson('/api/v1/inventory', [])
            ->assertStatus(422)
            ->assertJsonValidationErrors(['part_number', 'name', 'brand_id', 'location_id', 'type', 'stock', 'category_id', 'condition', 'status']);
    }
    ...
    public function update_berhasil_partial_update_hanya_satu_field()
    {
        ...
        $this->putJson("/api/v1/inventory/{$sparepart->uuid}", ['location_id' => $loc2->id])
            ->assertStatus(200);
        $this->assertDatabaseHas('spareparts', ['id' => $sparepart->id, 'location_id' => $loc2->id]);
    }
    ...
    public function update_gagal_jika_part_number_sudah_dipakai_barang_lain()
    {
        ...
        $this->putJson("/api/v1/inventory/{$sparepartB->uuid}", ['part_number' => 'PN-AAA'])
            ->assertStatus(422)
            ->assertJsonValidationErrors(['part_number']);
    }
    ...
    public function admin_dapat_mengakses_semua_endpoint_api()
    {
        ...
        $this->getJson("/api/v1/inventory/{$sparepart->uuid}")->assertStatus(200);
    }
