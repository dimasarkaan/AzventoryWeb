 ...
 */
class TesEndpointUnduhTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        $response = $this->get(route('inventory.qr.download', $this->sparepart));
    ...
    {
        $response = $this->actingAs($this->superadmin)
            ->get(route('inventory.qr.print', $this->sparepartWithQr));
    ...
    public function print_qr_mengembalikan_404_jika_tidak_ada_qr_code()
    {
        ...
        $response = $this->actingAs($this->superadmin)
            ->get(route('inventory.qr.print', $this->sparepart));
    ...
    {
        $response = $this->get(route('inventory.qr.print', $this->sparepartWithQr));
        $response->assertRedirect(route('login'));
    }
