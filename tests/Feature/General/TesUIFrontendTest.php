...
class TesUIFrontendTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function operator_tidak_dapat_melihat_tombol_edit_hapus_di_inventaris()
    {
        ...
        $response->assertDontSee(route('inventory.edit', $this->item));
        $response->assertDontSee('action="'.route('inventory.destroy', $this->item).'"', false); // Cek escape HTML string
    ...
    public function admin_dapat_melihat_edit_tetapi_tidak_dapat_melihat_tombol_hapus_permanen()
    {
        ...
        $response->assertDontSee(route('inventory.force-delete', $trashedItem->uuid));
        $response->assertDontSee(route('inventory.restore', $trashedItem->uuid));
    }
    ...
    public function operator_melihat_widget_dashboard_yang_berbeda_dengan_admin()
    {
        ...
        $responseOp->assertSee('Pinjaman Aktif');
