...
class TesKasusBatasInventarisTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function sistem_mencegah_stok_negatif_melalui_peminjaman()
    {
        ...
        $response = $this->actingAs($this->superAdmin)->post(route('inventory.borrow.store', $item), [
            'user_id' => $user->id,
    ...
    public function tidak_dapat_menghapus_item_dengan_peminjaman_aktif()
    {
        ...
        $response = $this->actingAs($this->superAdmin)->delete(route('inventory.destroy', $item));
    ...
    public function menggabungkan_stok_jika_nomor_part_duplikat_ada()
    {
        ...
        // Act
        // Simulasikan POST ke store dengan part_number sama dan spasi di awal/akhir
        $response = $this->actingAs($this->superAdmin)->post(route('inventory.store'), [
            'part_number' => '  EXISTING-001  ', // Spasi harusnya di-trim
            'name' => 'Duplicate Item',
