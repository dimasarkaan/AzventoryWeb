...
class TesPersetujuanStokTest extends TestCase
{
    use RefreshDatabase;
    ...
    #[Test]
    public function admin_dapat_melihat_persetujuan_tertunda()
    {
    ...
    #[Test]
    public function admin_dapat_menyetujui_permintaan_stok()
    {
        $item = Sparepart::factory()->create(['stock' => 10]);
    ...
    #[Test]
    public function admin_dapat_menolak_permintaan_stok()
    {
        $item = Sparepart::factory()->create(['stock' => 10]);
    ...
    public function halaman_approval_mendukung_filter_status_riwayat()
    {
        ...
        StockLog::create(['sparepart_id' => $item->id, 'user_id' => $this->operator->id, 'type' => 'masuk', 'quantity' => 3, 'reason' => 'Approved Request', 'status' => 'approved', 'approved_by' => $this->admin->id]);
        StockLog::create(['sparepart_id' => $item->id, 'user_id' => $this->operator->id, 'type' => 'masuk', 'quantity' => 2, 'reason' => 'Rejected Request', 'status' => 'rejected', 'approved_by' => $this->admin->id, 'rejection_reason' => 'Alasan Tolak']);

        // Default: Pending only
        $responsePending = $this->actingAs($this->admin)->get(route('inventory.stock-approvals.index'));
        ...
        // Filter: Approved
        $responseApproved = $this->actingAs($this->admin)->get(route('inventory.stock-approvals.index', ['status' => 'approved']));
        $responseApproved->assertDontSee('Pending Request');
        $responseApproved->assertSee('Approved Request');
        $responseApproved->assertSee($this->admin->name); // Check approver name visible

        // Filter: Rejected
        $responseRejected = $this->actingAs($this->admin)->get(route('inventory.stock-approvals.index', ['status' => 'rejected']));
        ...
        $responseAll = $this->actingAs($this->admin)->get(route('inventory.stock-approvals.index', ['status' => '']));
    ...
    #[Test]
    public function superadmin_tidak_dapat_mengakses_halaman_persetujuan_stok()
    {
        $superadmin = \App\Models\User::factory()->create(['role' => 'superadmin']);
