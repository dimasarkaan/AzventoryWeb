...
class TesOptimasiDanAksiInventarisTest extends TestCase
{
    use RefreshDatabase;
    ...
    #[Test]
    public function admin_tidak_dapat_menjalankan_hapus_massal()
    {
        ...
        foreach ($ids as $id) {
            $this->assertDatabaseHas('spareparts', ['id' => $id, 'deleted_at' => null]);
        }
    }
    ...
    public function admin_dibatasi_akses_restore_barang_dari_sampah()
    {
        ...
        $this->actingAs($this->admin)
            ->patch(route('inventory.restore', $item->uuid))
            ->assertForbidden();
    }
}
