...
class TesProfilTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_user_dapat_menghapus_akun_mereka(): void
    {
        ...
        User::factory()->create(['role' => \App\Enums\UserRole::SUPERADMIN]); // Ensure not the last one
    ...
    #[Test]
    public function test_admin_dan_operator_tidak_dapat_menghapus_akun_sendiri(): void
    {
        $admin = User::factory()->create(['role' => \App\Enums\UserRole::ADMIN]);
        $operator = User::factory()->create(['role' => \App\Enums\UserRole::OPERATOR]);

        $responseAdmin = $this->actingAs($admin)->from('/profile')->delete('/profile', ['password' => 'password']);
        $responseAdmin->assertRedirect('/profile')->assertSessionHas('error');
        $this->assertNotNull($admin->fresh());

        $responseOperator = $this->actingAs($operator)->from('/profile')->delete('/profile', ['password' => 'password']);
        $responseOperator->assertRedirect('/profile')->assertSessionHas('error');
