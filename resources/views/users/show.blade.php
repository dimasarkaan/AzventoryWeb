        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div class="flex gap-3 self-start md:self-auto">
                     <a href="{{ route('users.index') }}" class="btn btn-secondary flex items-center gap-2 whitespace-nowrap">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                        {{ __('ui.back') }}
                    </a>
                    <a href="{{ route('users.edit', $user) }}" class="btn btn-warning flex items-center gap-2 whitespace-nowrap">
                ...
                <div class="space-y-4">
                    <div class="card p-6 flex flex-col items-center text-center">
                        ...
                        <div class="w-full border-t border-secondary-100 pt-4 text-left space-y-3">
                            <div>
                                <span class="text-xs text-secondary-400 uppercase tracking-wider font-semibold block">{{ __('ui.job_title') }}</span>
                                <span class="text-secondary-800 font-medium">{{ $user->jabatan ?? '-' }}</span>
                            </div>
                            <div>
                                <span class="text-xs text-secondary-400 uppercase tracking-wider font-semibold block">{{ __('ui.joined_since') }}</span>
                ...
                <div class="lg:col-span-2 space-y-4">
                    <div class="card p-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <span class="text-xs text-secondary-400 uppercase tracking-wider font-semibold block mb-1">{{ __('ui.email_address') }}</span>
                            ...
                            <div>
                                <span class="text-xs text-secondary-400 uppercase tracking-wider font-semibold block mb-1">{{ __('ui.phone_number') }}</span>
                                @if($user->phone)
                                    <div class="flex items-center gap-2 text-secondary-900">
                                ...
                                @else
                                    <span class="text-secondary-400 italic">{{ __('ui.not_set') }}</span>
                                @endif
                            </div>

                            <div class="md:col-span-2">
                                <span class="text-xs text-secondary-400 uppercase tracking-wider font-semibold block mb-1">{{ __('ui.address') }}</span>
                        ...
                        <table class="table-modern w-full">
                            <thead>
                                <tr>
                                    ...
                                    <th>{{ __('ui.borrow_date') }}</th>
                                    <th>{{ __('ui.status') }}</th>
                                    <th>{{ __('ui.return_date') }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                @forelse($user->borrowings->sortByDesc('created_at') as $borrowing)
                                    <tr class="group hover:bg-secondary-50 transition-colors">
                                        ...
                                        <td>
                                            ...
                                            <div class="text-xs text-secondary-500">{{ __('ui.due_date') }}: {{ $borrowing->expected_return_at ? $borrowing->expected_return_at->translatedFormat('d F Y') : '-' }}</div>
                                        </td>
                                        <td>
                                            @if($borrowing->status === 'returned')
                                                <span class="badge badge-success">{{ __('ui.status_returned_badge') }}</span>
                                            @elseif($borrowing->status === 'borrowed')
                                                @if($borrowing->expected_return_at < now())
                                                    <span class="badge badge-danger">{{ __('ui.status_overdue') }}</span>
                                                @else
                                                    <span class="badge badge-warning">{{ __('ui.status_borrowed') }}</span>
                                                @endif
                                            @else
