<x-app-layout>
                        ...
                        <div class="card p-6">
                            ...
                            <div class="hidden md:block overflow-x-auto">
                                <table class="w-full text-sm text-left">
                                    ...
                                    @foreach($borrowings as $borrowing)
                                        ...
                                        @if($canView)
                                        <tr class="hover:bg-primary-50 transition-colors group cursor-pointer" onclick="window.location.href='{{ route('superadmin.inventory.borrow.show', $borrowing) }}'">
                                            ...
                                            <td class="px-4 py-3 text-right">
                                                ...
                                                @if($borrowing->returns->count() > 0 || !(($borrowing->status === 'borrowed' || $borrowing->remaining_quantity > 0) && auth()->id() === $borrowing->user_id))
                                                    <a 
                                                        href="{{ route('superadmin.inventory.borrow.show', $borrowing) }}"
                                                        @click.stop
                            ...
                            <div class="md:hidden space-y-4">
                                @foreach($borrowings as $borrowing)
                                    ...
                                    @if($canView)
                                    <div class="bg-white border border-secondary-200 rounded-xl p-4 shadow-sm hover:shadow-md transition-all cursor-pointer" onclick="window.location.href='{{ route('superadmin.inventory.borrow.show', $borrowing) }}'">
                                        <!-- Header: User & Status -->
            ...
            <div x-show="returnModalOpen" 
                ...
                <div class="flex min-h-screen items-center justify-center py-12 px-4 sm:px-6">
                    ...
                    <div x-show="returnModalOpen" 
                        ...
                        <!-- Form (Scrollable Body) -->
                        <form :action="`/superadmin/inventory/borrow/${selectedBorrowing}/return`" method="POST" enctype="multipart/form-data" class="flex flex-col flex-1 min-h-0" @submit.prevent="submitReturn">
                            @csrf
    ...
    @include('superadmin.inventory.partials.alpine_script')
</x-app-layout>
