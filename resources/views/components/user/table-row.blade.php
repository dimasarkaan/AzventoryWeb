...
<tr class="group hover:bg-secondary-50/60 transition-colors border-b border-secondary-50 last:border-b-0">
    ...
    <td class="px-4 py-3">
        <div class="flex items-center gap-3">
            <div class="h-10 w-10 rounded-full bg-secondary-100 flex-shrink-0 flex items-center justify-center text-secondary-500 overflow-hidden border border-secondary-200 group-hover:border-primary-200 transition-colors">
                @if($user->avatar)
            ...
            <div>
                <div class="font-medium text-secondary-900 group-hover:text-primary-600 transition-colors">{{ $user->name }}</div>
    ...
    <td class="px-4 py-3 text-sm text-secondary-600">{{ $user->jabatan ?? '-' }}</td>
    <td class="px-4 py-3">
        @php
            $roleColors = [
        ...
        @endphp
        <span class="inline-flex px-2.5 py-0.5 rounded text-xs font-medium border {{ $roleColors[$user->role->value] ?? 'text-secondary-700 bg-secondary-50 border-secondary-100' }}">
            {{ $user->role->label() }}
        </span>
    </td>
    <td class="px-4 py-3">
    ...
    <td class="px-4 py-3">
        <div class="flex items-center justify-end gap-2">
            @if($trash)
                @can('restore', $user)
                    <form action="{{ route('users.restore', $user->uuid) }}" method="POST" class="inline-block" novalidate>
                        @csrf
                        @method('PATCH')
                ...
                @endcan
            @else
                <a href="{{ route('users.show', $user) }}" class="btn btn-ghost p-2 text-secondary-600 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition-all" title="{{ __('ui.detail') }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                </a>
                @can('update', $user)
                    <a href="{{ route('users.edit', $user) }}" class="btn btn-ghost p-2 text-warning-600 hover:text-warning-700 hover:bg-warning-50 rounded-lg transition-all" title="{{ __('ui.edit') }}">
                ...
                @can('delete', $user)
                    <form action="{{ route('users.destroy', $user) }}" method="POST" class="inline-block" novalidate>
                        @csrf
                        @method('DELETE')
                        <button type="button" class="btn btn-ghost p-2 text-danger-600 hover:text-danger-700 hover:bg-danger-50 rounded-lg transition-all" title="{{ __('ui.delete_soft_tooltip') }}" onclick="confirmDelete(event)">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                        </button>
                    </form>
                @endcan
